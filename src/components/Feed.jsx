...
import { addFeed, updateUserStatus } from "../utils/feedSlice";
...
const Feed = () => {
  ...
  const handleSendRequest = async (status, userId) => {
    try {
      ...
      // Update Redux state for this user instead of removing
      dispatch(updateUserStatus({ userId, status }));

      // Update local filtered feed so UI updates instantly
  ...
  const handleSearch = () => {
    ...
    const filtered = feed.filter((u) => u.company?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleReferralSubmit = async () => {
    ...
    if (!isValidUrl(jobLink)) return alert("Please enter a valid job link");
    if (!isValidUrl(resumeLink)) return alert("Please upload a valid resume link");
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_260px] gap-8 lg:gap-12 justify-center items-start">
        ...
        <div className="flex flex-col gap-8 items-center w-full">
          {filteredFeed.length === 0 ? (
            ...
            filteredFeed.map((user, idx) => (
              <motion.div
                key={user._id}
                className="flex flex-col sm:flex-row items-center bg-slate-800/25 border border-gray-700 shadow-2xl rounded-2xl p-5 sm:p-6 w-full max-w-xl"
              ...
              >
                ...
                <div className="mt-4 sm:mt-0 sm:ml-6 text-center sm:text-left w-full">
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row gap-3 items-center">
                    {/* CONNECT BUTTON */}
                    <motion.button
                      disabled={user.connectionStatus === "interested"}
                      className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl font-semibold text-sm sm:text-base w-32 sm:w-auto transition-transform duration-200
                        ${
                      ...
                      `}
                      whileHover={user.connectionStatus !== "interested" ? { scale: 1.05 } : {}}
                      onClick={() => handleSendRequest("interested", user._id)}
                    >
                      {user.connectionStatus === "interested" ? "Requested" : "Connect"}
                    </motion.button>

                    {/* REFERRAL */}
                    <motion.button
                      className="px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl bg-gradient-to-r from-primary to-secondary text-gray-100 font-semibold text-sm hover:scale-105 w-36"
        ...
        <motion.aside
          className="hidden lg:block bg-slate-800/25 border border-gray-700 rounded-2xl p-5 shadow-2xl h-fit"
          initial={{ x: 60, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
        >
          ...
          <div className="flex flex-col gap-5 text-gray-300">
            ...
            <div className="bg-slate-800/50 rounded-xl p-4 shadow-md">
              ...
              <div className="mt-3 flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-gray-100">Arkaja Singh</p>
                  <p className="text-xs text-gray-400">Devops Engineer @ Reliance JIO</p>
                </div>
              </div>
            </div>
          </div>
